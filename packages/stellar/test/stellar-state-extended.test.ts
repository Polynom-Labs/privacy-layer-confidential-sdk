...
describe('StellarPrivacyClient pending claims state', () => {
  it('replaces pending claims page instead of duplicating owner rows', async () => {
    const { client } = await createTestClient();
    ...
    expect(claims.items[0]?.id).toBe('2');
    expect(await client.getPendingClaimsCount('G-OWNER')).toBe(1);
  });

  it('upserts pending claims by id', async () => {
    const { client } = await createTestClient();
    await client.upsertPendingClaims([
      {
        id: 'claim-a',
        owner: 'G-OWNER',
        asset: 'USDC',
    ...
    expect(claims.items.find((claim) => claim.id === 'claim-a')?.amount).toBe(15n);
  });
});

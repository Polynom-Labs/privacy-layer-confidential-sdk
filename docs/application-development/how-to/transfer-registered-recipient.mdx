...
After prepare, call `requiredSubmissionMethod(operation.prepared, zkConfigNonce)` from `@arcanetech/privacy-sdk-stellar/transact` (pass the pool’s nonce; SDK default is `3n`; demos use `7n`):

- `direct` — `await operation.execute({})`
- `relay` — do **not** call `execute()`; wire `@arcanetech/privacy-sdk-relay` as in the [Quick start](/integration/quick-start) (Frontend or Backend tab) demo

## Example

<Warning>
  Pool IDs, `G…` / `stpl1` addresses, and seed hex below are **tutorial fixtures**. The client can be created if `auditPublicKeyHex` is a valid BabyJubJub key (Setup uses the SDK bundled demo key). For a live transfer, use the [Quick start](/integration/quick-start) (Frontend or Backend tab) demo `.env`, or your own stand. See [Data sources](/integration/data-sources).
</Warning>
...
if (method === 'direct') {
  ...
  // Private sender: Protocol Relay. See Quick start demos.
}
```

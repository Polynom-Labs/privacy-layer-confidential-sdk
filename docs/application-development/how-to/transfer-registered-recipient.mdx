...
After prepare, call `requiredSubmissionMethod(operation.prepared, zkConfigNonce)` from `@arcanetech/privacy-sdk-stellar/transact` (pass the pool’s nonce; SDK default is `3n`):

...
To claim a discovered note, `fetchEscrowOutputNoteEvents` reads the output-note ciphertexts through the existing transact environment (do not pass an RPC URL). After proving the sweep, `unsignedEscrowAuthorizationForSweep` simulates the relayer invocation so the wallet can sign that authorization entry offline. The signed entry goes on the relay package; the wallet does not sign the transaction envelope.

...
// Implement store, submitDirect, finalizeLocalState, drainDeliveries,
// persistUserTransaction, and relayOperation in your app — see the
// Frontend / Backend demos for complete ports.
...
Build `relayOperation` from `operation.prepared` (`prepareRelayTransactPackageFromPrepared` + `serializeRelayPackage`, `submissionPath: 'relay'`). Complete port implementations: [Examples](/integration/quick-start).
...
A recipient who registers later can still find notes that were sent to their public wallet. Escrow discovery uses **blinded recipient tags**, not the retired pending-claim client API.

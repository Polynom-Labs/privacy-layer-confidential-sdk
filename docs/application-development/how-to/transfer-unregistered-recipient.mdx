...
// after createStellarPrivacyClient succeeds:
...
<Warning>
  Pool IDs, `G…` / `stpl1` addresses, and seed hex below are **tutorial fixtures**. The client can be created if `auditPublicKeyHex` is a valid BabyJubJub key (Quick start uses the SDK bundled demo key). `execute()` will not confirm on testnet with these values — use your own pool, registry, KYT, audit key, and wallet. See [Data sources](/integration/data-sources).
</Warning>
...
if (method !== 'relay') {
  throw new Error('Escrow sends must use Protocol Relay.');
}

// You implement these — they are not SDK exports.
...
Build `relayOperation` from `operation.prepared` (`prepareRelayTransactPackageFromPrepared` + `serializeRelayPackage`, `submissionPath: 'relay'`). See [Quick start Step 6](/integration/quick-start#step-6--protocol-relay-optional).
...
<CardGroup cols={2}>
  <Card title="Operations — Unregistered recipient" icon="arrow-right-arrow-left" href="/concepts/operations#transfer-to-unregistered-recipient-escrow-send">

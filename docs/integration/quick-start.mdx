...
<Tip>
  `loadDefaultStellarBrowserAssets()` loads the bundled SDK WASM. Circuit proving artifacts are resolved internally from the selected `zkConfigNonce` (Commitment V2 by default). `transactEnvironment` ties together network contracts, optional KYT, and your app's transaction signing hook.
</Tip>

---
title: Data sources
description: Production data flows vs documentation fixtures for SDK state
---

import { DataSourceLegend } from '../snippets/data-source-legend.jsx';

SDK state can be populated manually for tutorials, but production applications load domain data from **trusted services** and **chain sync**. This page separates the two.

<Warning>
  **Seed state** blocks in [Deposit](/application-development/how-to/deposit), [Withdraw](/application-development/how-to/withdraw), and [Transfer](/application-development/how-to/transfer-registered-recipient) how-to guides are **documentation fixtures**. They exist so examples run in isolation. Do not ship them as user onboarding flows.
</Warning>

<DataSourceLegend />

## Production data flow

```mermaid
flowchart LR
  AssetsCatalog[Asset catalog sync]
  ChainSync[Chain event sync]
  Deliveries[Delivery indexer]
  Registry[Registry status sync]
  WalletUX[Wallet UX flows]

  AssetsCatalog --> UpsertAssets[upsertAssets]
  ChainSync --> PoolState[setPoolMerkleState]
  Deliveries --> Records[upsertPrivateRecords]
  Registry --> Lookup[saveRegistryLookup]
  WalletUX --> WalletKeys[saveWalletPrivateAddress*]

  UpsertAssets --> SDKState[SDK state cache]
  PoolState --> SDKState
  Records --> SDKState
  Lookup --> SDKState
  WalletKeys --> SDKState
```

Your application orchestrates these writes **before** users submit operations, keeping prepare reliable.

## SDK method × typical source

| SDK write | Production source | Doc fixture |
| --- | --- | --- |
| `upsertAssets` | Your backend asset catalog | Hardcoded catalog row |
| `setPoolMerkleState` | Chain scanner or SDK sync after events | Static root and commitments |
| `upsertPrivateRecords` | Delivery polling / output-note decryption | Manual `coinNote` object |
| `saveRegistryLookup` | `checkRegistrationStatus` or registry poll | Hardcoded lookup row |
| `saveWalletPrivateAddressRecord` | Wallet private-address creation flow | Static `stpl1` address |
| `saveWalletPrivateAddressScalar` | Wallet signature during address setup | Static scalar hex |
| `setWalletDefaultPrivateAddressNonce` | Same wallet UX after the record/scalar exist | `{ owner, nonce: '1' }`. Spend prepare reads `$.wallet.defaultPrivateAddressNonce` — without this call it throws, it does not return `rejected`. |

## What runs automatically on execute

After a successful operation, the SDK updates local state without manual calls:

- mark spent records as consumed
- insert output and change records
- refresh pool commitment snapshot
- record delivery metadata when configured

Label these updates **SDK auto** in your mental model — not fixtures and not backend-fed.

## Stellar preset examples {#stellar-preset-examples}

For the shipped Stellar preset, production wiring typically looks like:

| Data | Stellar-specific source |
| --- | --- |
| Asset rows | Asset catalog from your backend (includes `clientContract` and `poolContract` Soroban IDs) |
| Pool Merkle state | Pool event scanner or client sync against Soroban RPC |
| Private records | Incoming delivery sync and output-note handling in your payment app |
| Registry lookup | On-chain registry simulate + `checkRegistrationStatus` |
| Escrow note discovery | Blinded recipient tags + `fetchEscrowOutputNoteEvents` after the recipient registers |
| Audit interpretation | Encrypted audit event indexing and interpretation in your stack |

<Warning>
  How-to **Seed state** sections use example `G...` and `stpl1...` addresses and static hex commitments. Treat them as copy-paste fixtures for docs only.
</Warning>

### Minimal Stellar production checklist

- [ ] Asset catalog synced from backend before showing balances
- [ ] Pool Merkle state kept current via scanner or post-tx sync
- [ ] Wallet private-address and scalar created through wallet UX, not hardcoded
- [ ] Registry status fetched before registered-recipient transfers
- [ ] Deliveries indexer populates `upsertPrivateRecords` for incoming notes
- [ ] Environment variables set for pool, registry, application ID, audit public key

## Where each config field comes from

Tutorial IDs in Quick start and how-to pages are **fixtures**. A live client must use the pool, registry, KYT, application ID, and audit key of **your** stand or Compliance application. Values currently present in this repo (they can rotate):

| Field | Tutorial fixture | SDK `bindings:generate` | Stellar Pay testnet stand |
| --- | --- | --- | --- |
| Pool | `CBVQTSTSIJ4UZMN5FQBFHPUIYSW3ATGSP57BZDJNKTOBNQODQTRGE4K5` | `CD56SAQXVPM3MWTTNVDBDVUEQC6DPRIIYHP67C3ICH6JGMLJSWWUFUON` | `CD56SAQXVPM3MWTTNVDBDVUEQC6DPRIIYHP67C3ICH6JGMLJSWWUFUON` |
| Registry | `CCO5QRTS5HFVPR356FFC4NI62IMBQNDGMOAXR4K5UVTV3YMRWDZGKXGC` | `CCO5QRTS5HFVPR356FFC4NI62IMBQNDGMOAXR4K5UVTV3YMRWDZGKXGC` | `CCBTGPQFV74MMRNML23UCG3LEGU2WA2S6JU67HVZDGEYTXR6KMTSSA4C` |
| KYT registry | `CKYTEXAMPLECONTRACTIDAAAAAAAAAAAAAAAAAAAAAA` | — | `CCOHV6N5Q6O3AWBZ66G5NWSG6J5UXVBOXJ3LIDBQ35WOPGTOMGQH4AYH` |
| KYT API | `https://example.invalid` | — | `https://stellar.arcane.finance/payment/api` |
| Application ID | `00000000-0000-0000-0000-000000000001` | — | `3520878299009890` |
| Audit public key | tutorial placeholder in Quick start / Setup | — | `044899603ebf8f88…c1ca8a6` |
| `zkConfigNonce` | omit (SDK default = Commitment V2, nonce `2`) | — | `0` (legacy stand) |
| Relay origin | unset → Relayer is never contacted | — | not set in the payment UI |

Do not copy the Pay stand `zkConfigNonce=0` into a new integration unless that stand is the target. Escrow sends without `relayConfig.origin` fail (`escrow_relay_unconfigured`); they are not downgraded to Direct.

## Related

<CardGroup cols={2}>
  <Card title="Domain model" icon="sitemap" href="/concepts/domain-model">
    What each state entity represents.
  </Card>
  <Card title="Operations" icon="arrow-right-arrow-left" href="/concepts/operations">
    Which entities each operation consumes.
  </Card>
  <Card title="Deposit how-to" icon="book" href="/application-development/how-to/deposit">
    Fixture seed state example.
  </Card>
  <Card title="State integration" icon="database" href="/integration/state-integration">
    Adapter libraries for persisting SDK state.
  </Card>
</CardGroup>

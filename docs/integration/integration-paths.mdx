...
1. Install `@arcanetech/privacy-sdk-stellar` and a state adapter — see [Quick start](/integration/quick-start) or [Install](/application-development/install). For private sender or escrow sends, also install [`@arcanetech/privacy-sdk-relay`](https://www.npmjs.com/package/@arcanetech/privacy-sdk-relay).
...
| Caller-supplied `submissionPath` on each private operation | `requiredSubmissionMethod(prepared)` in the Stellar preset |

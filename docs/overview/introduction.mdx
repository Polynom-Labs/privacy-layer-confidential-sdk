...
The SDK splits into these layers:

| Layer | Packages | Role |
| --- | --- | --- |
| **Core** | `@arcanetech/privacy-sdk-core` | Chain-agnostic intents, operation lifecycle, errors, progress events |
...
| **State** | `@arcanetech/privacy-sdk-state-*` | Pluggable persistence for SDK domain state |
...
3. **Integration** — [Getting access](/integration/getting-access) (npm + stand values), runtime paths, quick start, state adapters, production data sources.

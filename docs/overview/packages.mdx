...
| `@arcanetech/privacy-sdk-stellar/transact` | Stellar transaction helpers, including `requiredSubmissionMethod` and `buildBlindedRecipientTagChallengeMessage` |
...
- **`requiredSubmissionMethod(prepared)`** — Direct Submission vs Protocol Relay for a prepared operation (pending claims and positive public deposits stay direct; a zero public deposit uses relay). Escrow sends require relay and a zero public deposit; a positive public deposit is refused rather than downgraded.

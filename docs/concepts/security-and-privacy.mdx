...
This page lists what the SDK stores locally, what reaches Stellar, and what auditors and compliance services can see.
...
| **Stellar** | Commitments, nullifiers, disclosed fields, encrypted audit events |
...
When KYT is configured on the Stellar client, the SDK may submit a **passage inspect** request before submission. High-level payload contents include hashed public signals and passage metadata — not local note secrets.
...
- **Transaction signing** — signs the prepared Stellar transaction payload
